...
{
    public class HSMTCListener(GlobalSystemMediaTransportControlsSession session)
    {
        private GlobalSystemMediaTransportControlsSession _globalSMTCSession = session;
        public static async Task<HSMTCListener> CreateInstance()
        {
            var gsmtcsm = await GlobalSystemMediaTransportControlsSessionManager.RequestAsync();
            var smtcHelper = new HSMTCListener(gsmtcsm.GetCurrentSession());
